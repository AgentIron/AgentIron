...
- **THEN** AgentIron SHALL show a global error notification identifying which tab's MCP settings update failed
...
### Requirement: AgentIron MAY keep background diagnostics console-only
AgentIron MAY leave background/system diagnostic failures console-only when they are not directly caused by a visible user action or occur outside user-facing workflows.
...
- **THEN** AgentIron SHALL log the failure to the console and MAY omit user-facing notifications
...
- **THEN** AgentIron MAY allow console-only diagnostic logging for that condition
